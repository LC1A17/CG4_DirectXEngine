...
//静的メンバ変数の実体
const std::string Model3d::baseDirectory = "Resources/";
ID3D12Device* Model3d::dev = nullptr;
UINT Model3d::descriptorHandleIncrementSize = 0;

//静的初期化
void Model3d::StaticInitialize(ID3D12Device* dev)
{
	Model3d::dev = dev;

	Mesh::StaticInitialize(dev);//メッシュの静的初期化
}

//OBJファイルからメッシュ生成
Model3d* Model3d::CreateFromOBJ(const std::string& modelname)
{
	//メモリ確保
...
//初期化処理
void Model3d::Initialize(const std::string& modelname)
{
...
//マテリアル登録
void Model3d::AddMaterial(Material* material)
{
	//コンテナに登録
